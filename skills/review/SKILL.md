---
name: review
description: 'Run a standard Claude Code review of local git changes in this repository. Args: --wait, --background, --base <ref>, --scope <auto|working-tree|branch>, --model <model>, --effort <auto|low|medium|high|xhigh|max>. Defaults to opus with CLI-selected effort. Use as the default path for ordinary code-review requests when the user did not explicitly ask for stronger adversarial scrutiny or for Claude to own the implementation work.'
---
...
Supported arguments: `--wait`, `--background`, `--base <ref>`, `--scope auto|working-tree|branch`, `--model <model>`, `--effort <auto|low|medium|high|xhigh|max>` (defaults: model=opus, effort=auto delegated to Claude CLI)

---
name: rescue
description: 'Delegate a substantial diagnosis, implementation, or follow-up task to Claude Code through the tracked-job runtime. Args: --background, --wait, --resume, --resume-last, --fresh, --write, --model <model>, --effort <auto|low|medium|high|xhigh|max>, --prompt-file <path>, [task text]. Defaults to opus with CLI-selected effort. Use when Claude should investigate or change things, not when the user only wants review findings.'
---
...
Supported arguments: `--background`, `--wait`, `--resume`, `--resume-last`, `--fresh`, `--write`, `--model <model>`, `--effort <auto|low|medium|high|xhigh|max>`, `--prompt-file <path>`, plus free-text task text
